...
const btnStyleClass = "p-2 text-white " +
...
const Navigator = ({maxIdx}) => {
  ...
  return (
    <div
      className={"flex justify-between w-full bg-fuchsia-500 p-4"}
    >
      <button type="button"
              disabled={resumeData.navigationActiveIdx === 0}
              onClick={() => {
                setResumeData({...resumeData, navigationActiveIdx: resumeData.navigationActiveIdx - 1});
              }}
              aria-label="Back"
              className={btnStyleClass}>
        Back
      </button>
      <NavigationDots count={maxIdx + 1} activeIndex={resumeData.navigationActiveIdx}/>
      <button type="button"
